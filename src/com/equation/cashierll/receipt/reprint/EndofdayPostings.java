...
public class EndofdayPostings extends JPanel implements ActionListener {
	...
	public EndofdayPostings(ResultSet rs, ResultSet rs1, Statement stm, Statement stmt, JTabbedPane tabs,
			JFrame frame) {
		this.rs = rs;
		this.rs1 = rs1;
		this.stm = stm;
	...
	public final void init() {
		...
		JLabel amlbl = new JLabel("End Of Day Amount $:");
	...
	public void insertTab() {
		EventQueue.invokeLater(() -> {
			...
			for (int a = 0; a < numberoftabs; a++) {
				if (tabs.getTitleAt(a).trim().equals("End Of Day Postings")) {
					exist = true;
					break;
				}
			}
			if (!exist) {
				tabs.addTab("End Of Day Postings   ", null, this, "Posting End Of Day");
	...
	public void submit() {
		String query = "SELECT * FROM endofday WHERE cashierid  = '" + idstring + "' AND date = '"
		...
		try {
			...
			double sum = bal + amo;
			int with = 0;
			String details = "Cashier id " + idstring + " End of day cash Deposit.Approved By supervisor";
			String query11 = "INSERT INTO ledger(date,time,deposit,withdrawal,balance,details)VALUES('"
					+ new SetDateCreated().getDate() + "','" + new SetDateCreated().getTime() + "','" + amo + "','"
					+ with + "','" + sum + "','" + details + "')";
			stm.execute(query11);
		} catch (Exception ee) {
			JOptionPane.showMessageDialog(frame, "Amount rejected. \nIt does not match cashier id End Of Day amount",
