...
public class GenerateRecoPDF {
	String tablename = "Reconciliation Report";;
	String date, date1;
	JFrame frame;
	Object[][] tableData;
	JTable jtable;

	public GenerateRecoPDF(String date, String date1, JFrame frame, Object[][] tableData, JTable jtable) {
		this.date = date;
		this.date1 = date1;
		this.frame = frame;
	...
	public void generatePDF() {
		try {
			...
			document.add(new Paragraph("Reconciliation Statement Generated",
			...
			PdfPCell cell = new PdfPCell(new Paragraph(("Reconciliation Statement").toUpperCase(),
			...
			for (int i = 0; i < jtable.getColumnCount(); i++) {
				...
				table.addCell(new Phrase(Header.header[i],
			...
			for (int rows = 0; rows < jtable.getRowCount(); rows++) {
				for (int cols = 0; cols < jtable.getColumnCount(); cols++) {
					table.addCell("" + tableData[rows][cols]);
				}
			}
			...
			JOptionPane.showMessageDialog(frame, "File access Error\n" + ee.getMessage(), "Error",
